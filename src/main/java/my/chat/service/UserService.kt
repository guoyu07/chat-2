...
class UserService {
    ...
    fun login(user: User, username: String): User? {
        ...
        if (ReUtil.isMatch("((13[0-9])|(14[5|7])|(15([0-3]|[5-9]))|(18[0,5-9]))\\d{8}", username))
            ...
            sb.append(" username = ?")
        return userDao.findFirst(sb.toString(), username)
    }
    ...
    fun verifyMailExists(mail: String): List<User>? = userDao.find("select id,status,email from user where email = '${mail}'")
